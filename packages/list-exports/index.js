...
// NEW: glob for expanding RHS patterns
...
function addFullPath(
    ...
    // NOTE: packlist can exclude dev/test files, so use on-disk existence here.
...
async function forEachExportEntry([lhs, maybeRHS], conditionChain, {
    ...
    if (typeof lhs === 'string' && typeof maybeRHS === 'string') {
        const leftStars = (lhs.match(/\*/g) || []).length;
        const rightStars = (maybeRHS.match(/\*/g) || []).length;

        // Node-style behavior: * counts must match (each * is a single path segment)
        if (leftStars !== rightStars) {
            problems.add(`Mismatched number of '*' in "${lhs}": "${maybeRHS}". Both sides must have the same number of '*'.`);
            return false;
        }

        // Starred patterns: expand RHS glob to files, capture segments, rebuild LHS, and add mapping
        if (leftStars > 0) {
            // Build capture regex from RHS to extract star segments
            const rhsCaptureRe = new RegExp('^' + escapeRegex(maybeRHS).replace(/\\\*/g, '([^/]+)') + '$');

            const rhsPatternRel = maybeRHS.replace(/^\.\//, '').replace(/\\/g, '/'); // e.g., "src/good/*.js"

                // Debug: show glob search details
                console.log('[GLOB] cwd =', packageDir);
                console.log('[GLOB] pattern =', rhsPatternRel);

                const matchedRelFiles = fg.sync(rhsPatternRel, {
                cwd: packageDir,
                onlyFiles: true,
                dot: false,
                ...
                console.log('[GLOB] matched =', matchedRelFiles);


            let mappedSomething = false;

            for (const relNoDot of matchedRelFiles) {
              const rel = './' + relNoDot.replace(/\\/g, '/'); // e.g. "./src/good/foo.js"

              const m = rel.match(rhsCaptureRe);
              if (!m) continue;

                let i = 0;
                  const lhsResolved = lhs.replace(/\*/g, () => m[++i] || '');
  ...
  if (ok) mappedSomething = true;
}


            if (!mappedSomething) {
                problems.add(`“${lhs}”: ${maybeRHS} does not appear to match any files!`);
            }
            return mappedSomething;
        }
    }

    // fallback to original logic if no stars are involved
...
if (require.main === module) {
    (async () => {
        const result = await module.exports(process.argv[2]);
        console.log('\nProblems found:');
